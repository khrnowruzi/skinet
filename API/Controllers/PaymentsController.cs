...
public class PaymentsController(IPaymentService paymentService,
    IUnitOfWork uow, ILogger<PaymentsController> logger,
    IConfiguration config, IHubContext<NotificationHub> hubContext) : BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        {
            logger.LogError(ex, "Stripe webhook erro");
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        if (intent.Status == "succeeded")
        {
            ...
            var order = await uow.Repository<Core.Entities.OrderAggregate.Order>().GetEntityWithSpec(spec)
